        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    output_dir = os.path.join(args.output_data_dir, 'checkpoint-{}'.format(global_step))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    eval_outputs_dirs = (args.output_data_dir, args.output_data_dir + '-MM') if args.task_name == "mnli" else (args.output_data_dir,)

    results = {}
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = data.load_and_cache_examples(args, eval_task, tokenizer, evaluate=True)
...
def parse_args():
    return config_args.parse_args()


def main():
    args = parse_args()

    if os.path.exists(args.output_data_dir) and os.listdir(args.output_data_dir) and args.do_train and not args.overwrite_output_dir:
        raise ValueError("Output directory ({}) already exists and is not empty. Use --overwrite_output_dir to overcome.".format(args.output_data_dir))
    ...
    train_flag=True
    if train_flag:
        train_dataset = data.load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False)
        global_step, tr_loss = train(args, train_dataset, model, tokenizer)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)


    # Saving best-practices: if you use defaults names for the model, you can reload it using from_pretrained()
    if train_flag and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        # Create output directory if needed
        if not os.path.exists(args.output_data_dir) and args.local_rank in [-1, 0]:
